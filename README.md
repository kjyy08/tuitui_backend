...
| 순서 |                             설명                             |                     비고                     |
|:--:|:----------------------------------------------------------:|:------------------------------------------:|
| 1  |         유저가 소셜 로그인에 성공하여 필요한 정보를 담아 로그인 요청을 보냅니다.          | grant_type, Access/Refresh 토큰,<br> 계정 정보 등 |   
| 2  |                컨트롤러에서 서비스로 토큰 검증 요청을 보냅니다.                 |                                            |
| 3  | 서비스에서 실제 SNS 이용자인지 검증하기 위해<br> SNS API 서버로 유저 검증 요청을 보냅니다. |          SDK에서 발급받은 Access 토큰 이용           |
| 4  |                   유저 검증에 성공하면 토큰을 생성합니다.                   |                                            |   
| 5  |             토큰 재발급에 사용할 Refresh 토큰을 DB에 저장합니다.             |       Refresh 토큰, 계정 정보, 만료일을 DB에 저장       |   
